...
import static com.example.ahmed.octopusmart.App.Config.MY_PERMISSIONS_REQUEST_CAMERA;
import static com.example.ahmed.octopusmart.App.Config.MY_PERMISSIONS_REQUEST_STORAGE;
import static com.example.ahmed.octopusmart.App.Config.REQUEST_IMAGE_CAPTURE;


public class UplodaImagesActivity extends LoadingDialogActivity {

    private ArrayList<String> photoPaths = new ArrayList<>();
    private ArrayList<File> photos_result_uri_array  = new ArrayList<>();
    public  static   int MAX_IMAGE_COUNT = 1 ;
    onUploadResponse onUploadResponse ;
    public interface  onUploadResponse{
        void onSuccess(ResponseBody imgs_urls);
        void onFailure();
    }
    ...
    private void upload_Files() {
       ...
       call = Injector.UploadApi().upload(
        ...
        call.enqueue(new Callback<MultiReponseItem>() {
            @Override
            public void onResponse(Call<MultiReponseItem> call,
                                   Response<MultiReponseItem> response) {

                if (response.isSuccessful() && response.body().getImages() !=null && !response.body().getImages().isEmpty()){
                    Log.e("OkHttp_upload_exp", "success");
                    onUploadResponse.onSuccess(response.body());


                }else if ( response.isSuccessful()&& response.body().getImage_path() !=null &&  !response.body().getImage_path().isEmpty()){
                    Log.e("OkHttp_upload_exp", "success  e" + response.body().getImage_path());
                    ArrayList<String> strings = new ArrayList<String>();
                    strings.add(response.body().getImage_path()) ;
                    response.body().images = strings;
                    onUploadResponse.onSuccess(response.body());
                }else {
            ...
            @Override
            public void onFailure(Call<MultiReponseItem> call, Throwable t) {
