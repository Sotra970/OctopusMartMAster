...
public class Validation {
    ...
    static  public boolean isEditTextEmpty(EditText editText){
        ...
        {
            editText.setError(Appcontroler.getInstance().getApplicationContext().getString(R.string.empty_error));
    ...
    static  public boolean isEditTextEmpty(EditText editText , TextInputLayout textInputLayout){
        if (TextUtils.isEmpty(editText.getText().toString()))
        {
            textInputLayout.setError(Appcontroler.getInstance().getApplicationContext().getString(R.string.empty_error));
            editText.requestFocus();
            return true;
        }
        else{
   ...
   static  public boolean isSpinnerEmpty(Spinner spinner , String ID) {
        ...
        if ( ID == null  || ID.equals("0") ){
            textView.setError(Appcontroler.getInstance().getApplicationContext().getString(R.string.empty_error));
    ...
    static public boolean isEmailNotValid(EditText editText){
        if (!isEditTextEmpty(editText))
        {
            ...
            {
                editText.setError(Appcontroler.getInstance().getApplicationContext().getString(R.string.email_error));
                editText.requestFocus();
                return true;
            }
            else
    ...
    static public boolean isEmailNotValid(EditText editText , TextInputLayout textInputLayout){
        if (!isEditTextEmpty(editText , textInputLayout))
        {
            if (!editText.getText().toString().matches("^\\w+([-+.']\\w+)*@\\w+([-.]\\w+)*\\.\\w+([-.]\\w+)*$"))
            {
                textInputLayout.setError(Appcontroler.getInstance().getApplicationContext().getString(R.string.email_error));
                editText.requestFocus();
                return true;
            }
            else{
    ...
    static public boolean validateLink(EditText editText){
        if (!isEditTextEmpty(editText))
        {
            ...
            {
                editText.setError(Appcontroler.getInstance().getApplicationContext().getString(R.string.link_error));
                editText.requestFocus();
                return true;
            }
            else
    ...
    static public boolean validatePhone(EditText editText){
        if (!isEditTextEmpty(editText))
        {
            ...
            if (!start_with_plus_or0 || !start_with_11_12_10_15 )
            {
                ...
                editText.setError(Appcontroler.getInstance().getApplicationContext().getString(R.string.phone_error));
                editText.requestFocus();
                return true;
            }
            else{
    ...
    static public boolean validatePhone(EditText editText , TextInputLayout textInputLayout){
        if (!isEditTextEmpty(editText))
        {
            ...
            {
                Log.e("phone" , "notvalid");
                textInputLayout.setError(Appcontroler.getInstance().getApplicationContext().getString(R.string.phone_error));
                editText.requestFocus();
                return true;
            }
            else{
    ...
    public static boolean validateCheckbox(CheckBox terms_check_bx) {
        if (!terms_check_bx.isChecked()){
            terms_check_bx.setError(Appcontroler.getInstance().getString(R.string.requiered));
